"""
Prompt definitions for VLM-as-judge collision evaluation (v2).

Changes from prompts.py (v1):
  - COMMON_HEADER: Added 3 new EVALUATION GUIDANCE bullets clarifying that
    only manipulator-to-item contact counts, pre-existing clutter is not
    collision, and items must change position within future frames.
  - CONTACT_ADDENDUM: Added IMPORTANT section explaining baseline contact
    spots from gravity/resting forces and how to compare context vs future.
  - P1-P5 bodies: unchanged from v1.
...
EVALUATION GUIDANCE:
- You are evaluating ONLY whether the manipulator/arm makes \
physical contact with an item. Contact between items, or \
between items and the table surface, does NOT count as a \
collision. The question is specifically: does the \
manipulator touch or hit an item?
- Items may already be in displaced or scattered positions \
from earlier interactions before the future window. A \
messy or cluttered scene does NOT indicate collision. \
Look for NEW movement or displacement of items that \
occurs WITHIN the future frames (frames 9\u201316), not \
pre-existing scene state.
- Ask yourself: between frame 9 and frame 16, do any items \
change position? If items remain in the same positions \
throughout all future frames, there is no collision, \
regardless of how close the manipulator appears or how \
scattered the scene looks.
