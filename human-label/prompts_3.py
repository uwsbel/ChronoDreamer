"""
Prompt definitions for VLM-as-judge collision evaluation (v3).

Changes from prompts_2.py (v2):
  - COMMON_HEADER: Completely restructured into concise labeled sections
    (SCENE, CAMERA, WHAT COUNTS AS COLLISION). Key addition: explains that
    ego-camera motion makes ALL items appear to shift, so the VLM must check
    for RELATIVE movement between items, not absolute pixel shift.
  - CONTACT_ADDENDUM: unchanged from v2.
  - P1-P5 bodies: unchanged from v2.
...
suddenly displaced near the manipulator without a visible \
contact moment, collision likely occurred between frames.
  \u2022 If the table appears to shrink or recede, the arm is \
lifting \u2014 collision is less likely.
  \u2022 When in doubt, favor a lower score. Only assign 4 or 5 \
when evidence is clear and sustained across multiple \
future frames."""
