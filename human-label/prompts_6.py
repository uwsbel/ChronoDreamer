"""
Prompt definitions for VLM-as-judge collision evaluation (v6).

Changes from prompts_5.py (v5):
  - COMMON_HEADER (SCENE): Clarifies 16 frames are arranged left-to-right in
    time order; CONTEXT is described as ground-truth simulation output and a
    reliable layout reference; notes that visual clarity may vary in FUTURE
    frames and instructs focusing on physical evidence over image sharpness.
  - CAMERA, WHAT COUNTS AS COLLISION, DEFAULT ASSUMPTION, CONTACT_ADDENDUM,
    OUTPUT_INSTRUCTION_STANDARD, P1-P5: unchanged from v5.
...
motion) rather than image sharpness when evaluating \
collision.
